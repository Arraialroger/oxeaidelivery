import { usePlatformSystemHealth } from '@/hooks/usePlatformSystemHealth';
...
export function PlatformHealthPanel() {
  const { events, loadingEvents, counts, lastCritical, avgConfirmationTime, hourlyData, testAlert } = usePlatformSystemHealth();
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <span className={`text-3xl font-bold ${counts.critical > 0 ? 'text-red-600' : 'text-green-600'}`}>{counts.critical}</span>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <span className={`text-3xl font-bold ${counts.warning > 0 ? 'text-yellow-600' : 'text-muted-foreground'}`}>{counts.warning}</span>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            {avgConfirmationTime !== null && avgConfirmationTime !== undefined ? (
              <div className="flex items-baseline gap-1">
                <span className="text-3xl font-bold">{avgConfirmationTime < 60 ? avgConfirmationTime : Math.round(avgConfirmationTime / 60)}</span>
                <span className="text-sm text-muted-foreground">{avgConfirmationTime < 60 ? 'seg' : 'min'}</span>
              </div>
            ) : (
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-base"><Zap className="w-5 h-5" /> Teste de Alerta</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-4">
            <Button onClick={() => testAlert.mutate()} disabled={testAlert.isPending} variant="outline">
              {testAlert.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <HeartPulse className="w-4 h-4 mr-2" />}
              Disparar alerta de teste
            </Button>
            <p className="text-xs text-muted-foreground">Cria um evento crítico global → enfileira no Telegram.</p>
          </div>
        </CardContent>
      </Card>
      ...
      {hourlyData.length > 0 && (
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-base">Falhas por Hora (24h)</CardTitle></CardHeader>
          <CardContent>
            <div className="h-[200px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={hourlyData}>
                  <CartesianGrid strokeDasharray="3 3" className="opacity-30" />
                  <XAxis dataKey="hour" tickFormatter={(v) => new Date(v).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })} fontSize={11} />
                  <YAxis allowDecimals={false} fontSize={11} />
                  <Tooltip labelFormatter={(v) => new Date(v as string).toLocaleString('pt-BR')} contentStyle={{ fontSize: 12 }} />
