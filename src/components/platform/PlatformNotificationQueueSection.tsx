import { usePlatformNotificationQueue } from '@/hooks/usePlatformNotificationQueue';
...
import { Bell, Clock, Loader2, Activity, Send, Zap } from 'lucide-react';
...
export function PlatformNotificationQueueSection() {
  const {
    notifications,
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center gap-4">
            <Button onClick={() => triggerHealthCheck.mutate()} disabled={triggerHealthCheck.isPending} variant="outline">
              {triggerHealthCheck.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Activity className="w-4 h-4 mr-2" />}
              Disparar Health-check manual
            </Button>
            <p className="text-xs text-muted-foreground">Executa o health-check e verifica alertas.</p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center justify-between text-base">
            <span className="flex items-center gap-2"><Bell className="w-5 h-5" /> Status das Notificações</span>
            <div className="flex items-center gap-2">
              ...
              <Select value={channelFilter || 'all'} onValueChange={(v) => setChannelFilter(v === 'all' ? null : v)}>
                <SelectTrigger className="w-[120px] h-8"><SelectValue placeholder="Canal" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">Todos</SelectItem>
                  ...
                  {notifications.map((n) => {
                    ...
                    return (
                      <TableRow key={n.id} className={n.status === 'failed' ? 'bg-destructive/5' : ''}>
                        ...
                        <TableCell><Badge variant={cfg.variant} className={cfg.className}>{cfg.label}</Badge></TableCell>
                        ...
                        <TableCell className="text-xs max-w-[200px] truncate text-destructive">{n.error_message || '—'}</TableCell>
                      </TableRow>
                    );
                  })}
                </TableBody>
