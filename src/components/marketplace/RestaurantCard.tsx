...
export function RestaurantCard({ restaurant }: RestaurantCardProps) {
  const { isOpen, isLoading: statusLoading, nextOpenTime, nextCloseTime, closingSoon, closingVerySoon, openingVerySoon } = useRestaurantOpenStatus(
    restaurant.id,
    restaurant.settings
  );
          ...
          {statusLoading ? (
            ...
            <Badge
              ...
              className={`absolute top-2 right-2 ${
                isOpen 
                  ? closingSoon
                    ? `bg-amber-500/90 text-white border-0 ${closingVerySoon ? 'animate-pulse' : ''}`
                    : 'bg-green-500/90 text-white border-0'
                  : `bg-muted text-muted-foreground ${openingVerySoon ? 'animate-pulse' : ''}`
              }`}
            >
              {isOpen 
